### Example configuration for deploying a new, multi-peer, Bowtie site within a new VPC and Subnet.
...
dns_zone_name   = "aws.bowtie.example.com" # The DNS zone name to be used for the controller (combines with controller_name to create FQDN).
controller_name = ["c0", "c1"] # The name of the controllers to be created (combines with dns_zone_name to create FQDN).
eip_addresses   = ["0.0.0.0", "0.0.0.1"] # Replace with the EIP addresses to be associated with each controller.
...
bowtie_username = "admin@aws.bowtie.example.com" # The username to be used for the Bowtie site.
...
### Example cloud-init file that should reside in /modules/compute/cloud-init-join-cluster.yaml
